...
<template>
  <div class="content">
    <div class="container">
      ...
      <div v-if="search != ''">
        <router-link v-for="(value, index) in history" :key="index" :to="{ name: 'history-detail', params: { id_order: value.id_order } }"
      ...
      <div v-else>
        <div v-for="item in data.list" :key="item.id" class="border-bottom mb-2 pb-2">
          ...
          <div class="collapse my-2 text-start" :id="'collapse_history' + item.id">
            <div v-if="item.id == 1">
              <router-link v-for="(value, index) in today" :key="index" :to="{ name: 'history-detail', params: { id_order: value.id_order } }"
                class="d-flex align-items-start justify-content-between mb-2">
                <div>
                  <p class="m-0">{{ value.nama_order }}</p>
                  <p class="m-0 sub sub-text">{{ value.type_order }}</p>
            ...
            <div v-else-if="item.id == 2">
              <router-link v-for="(value, index) in yesterday" :key="index" :to="{ name: 'history-detail', params: { id_order: value.id_order } }"
                class="d-flex align-items-start justify-content-between mb-2">
                <div>
                  <p class="m-0">{{ value.nama_order }}</p>
                  <p class="m-0 sub sub-text">{{ value.type_order }}</p>
            ...
            <div v-else-if="item.id == 3">
              <router-link v-for="(value, index) in this_week" :key="index" :to="{ name: 'history-detail', params: { id_order: value.id_order } }"
                class="d-flex align-items-start justify-content-between mb-2">
                <div>
                  <p class="m-0">{{ value.nama_order }}</p>
                  <p class="m-0 sub sub-text">{{ value.type_order }}</p>
            ...
            <div v-else-if="item.id == 4">
              <router-link v-for="(value, index) in last_week" :key="index" :to="{ name: 'history-detail', params: { id_order: value.id_order } }"
                class="d-flex align-items-start justify-content-between mb-2">
                <div>
                  <p class="m-0">{{ value.nama_order }}</p>
                  <p class="m-0 sub sub-text">{{ value.type_order }}</p>
            ...
            <div v-else-if="item.id == 5">
              <router-link v-for="(value, index) in this_month" :key="index" :to="{ name: 'history-detail', params: { id_order: value.id_order } }"
                class="d-flex align-items-start justify-content-between mb-2">
                <div>
                  <p class="m-0">{{ value.nama_order }}</p>
                  <p class="m-0 sub sub-text">{{ value.type_order }}</p>
            ...
            <div v-else-if="item.id == 6">
              <router-link v-for="(value, index) in last_month" :key="index" :to="{ name: 'history-detail', params: { id_order: value.id_order } }"
                class="d-flex align-items-start justify-content-between mb-2">
                <div>
                  <p class="m-0">{{ value.nama_order }}</p>
                  <p class="m-0 sub sub-text">{{ value.type_order }}</p>
